...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = MF(self.num_users, self.num_items, np.int(rank), np.int(batch_size), lamb=lam, alpha=alpha,
...
class MF(object):
    ...
    def _build_graph(self):
        ...
        with tf.compat.v1.variable_scope('mf'):# 声明变量作用域
            # 声明输入数据的Placeholder
            ...
            # 定义需要学习的参数Variable
            zc_user_embeddings = tf.compat.v1.Variable(tf.compat.v1.random_normal([self._num_users, self._embed_dim],
            ...
            self.zc_user_embeddings = tf.compat.v1.concat([zc_user_embeddings, user_zero_vector], 1)

            zc_item_embeddings = tf.compat.v1.Variable(tf.compat.v1.random_normal([self._num_items, self._embed_dim],
                                                             stddev=1 / (self._embed_dim ** 0.5), dtype=tf.float32))
            item_zero_vector = tf.compat.v1.get_variable(
                'item_zero_vector', [self._num_items, self._embed_dim],
                initializer=tf.compat.v1.constant_initializer(0.0, dtype=tf.float32), trainable=False)

            self.zc_item_embeddings = tf.compat.v1.concat([zc_item_embeddings, item_zero_vector], 1)

            

            with tf.compat.v1.variable_scope("mf_loss"):# 声明变量作用域
                zc_users = tf.nn.embedding_lookup(self.zc_user_embeddings, self.user_idx)
                zc_items = tf.nn.embedding_lookup(self.zc_item_embeddings, self.item_idx)
                zc_x_ij = tf.reduce_sum(tf.multiply(zc_users, zc_items), axis=1)

                mf_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=zc_x_ij, labels=self.label))

            with tf.compat.v1.variable_scope('l2_loss'):
                unique_user_idx, _ = tf.unique(self.user_idx)
                unique_users = tf.nn.embedding_lookup(self.zc_user_embeddings, unique_user_idx)

                unique_item_idx, _ = tf.unique(self.item_idx)
                unique_items = tf.nn.embedding_lookup(self.zc_item_embeddings, unique_item_idx)
    ...
    def train_model(self, matrix_train, _matrix_train, matrix_valid, matrix_test, epoch=100, metric='AUC', topK=50,
        ...
        for i in tqdm(range(epoch)):
            ...
            # 运行模型，同时获取模型的损失函数值
            _ ,loss= self.sess.run([self._train,self._loss], feed_dict=feed_dict)
            # 获取当前模型的隐向量矩阵
            RQ, Y = self.sess.run([self.zc_user_embeddings, self.zc_item_embeddings])
            # 对测试集进行评估，计算评估指标
...
def mf(matrix_train, matrix_valid, matrix_test, embeded_matrix=np.empty(0), iteration=500, lam=0.01, alpha=0.10273626485512183,
    ...
    if searcher == 'grid':
        ...
        model = MF(m, n, rank, batch_size, lamb=lam, alpha=alpha, gamma=gamma, learning_rate=learning_rate,
